...
import ANSRAssessment from "./ANSRAssessment";
  ...
  return (
    <div style={{ minHeight: "100vh", color: "#2C2C2C", position: "relative" }}>
      ...
      <div style={{ position: "relative", zIndex: 2, opacity: vis ? 1 : 0, transition: "opacity 0.9s ease", maxWidth: 600, margin: "0 auto", padding: "0 28px" }}>
        ...
        {/* Testimonial */}
        <div style={{ marginBottom: 72, maxWidth: 480, margin: "0 auto 72px", textAlign: "center" }}>
          <Orbit color={c} size={48} />
          <div style={{ height: 24 }} />
          <div style={{ borderLeft: `2px solid ${c}`, paddingLeft: 24, textAlign: "left", display: "inline-block", maxWidth: 420 }}>
            <p style={{ fontFamily: "'EB Garamond', serif", fontSize: 17, color: "#4A4540", lineHeight: 1.95, fontStyle: "italic", marginBottom: 20 }}>
            ...
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "#6B6560", letterSpacing: "0.04em", fontWeight: 500 }}>
              Delphine V.</p>
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: "#9B9590", letterSpacing: "0.04em", marginTop: 2 }}>
              SVP & GM, Big Tech · Switzerland</p>
          </div>
        </div>
...
export default function App() {
  if (typeof window === "undefined") return <ProfileLP />;
  const path = window.location.pathname;
  if (path.includes("delivery")) return <ProfileDelivery />;
  if (path.includes("assessment")) return <ANSRAssessment />;
  return <ProfileLP />;
}
